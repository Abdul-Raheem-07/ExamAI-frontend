...
import { Loader2, CheckCircle, AlertCircle, RefreshCw, Edit3, ArrowLeft, Brain, Clock, CheckCircle2, XCircle } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from 'recharts';

const StatusBadge = ({ status }) => {
  const map = {
    Pending:    { cls: 'badge badge-yellow', icon: <Clock size={10} /> },
    Processing: { cls: 'badge badge-indigo', icon: <RefreshCw size={10} style={{ animation: 'spin 0.8s linear infinite' }} /> },
...
const EvaluationResult = () => {
  ...
  const fetchStatus = useCallback(async () => {
    ...
    } catch { toast.error('Failed to fetch status'); setPolling(false); }
  ...
  if (!submission) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)' }}>Submission not found.</div>
  );

  const confidence = submission.confidence || 0;
  const confData   = [{ name: 'Confidence', value: confidence }, { name: 'Uncertainty', value: 100 - confidence }];
    ...
    <div className="page-wrapper">
      <div className="page-content">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
          <button onClick={() => navigate(backPath)} className="btn btn-ghost" style={{ width: 36, height: 36, padding: 0, justifyContent: 'center', flexShrink: 0 }}>
            <ArrowLeft size={16} />
          </button>
          <div style={{ flex: 1 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: '0.8125rem', margin: 0 }}>
              {submission.examId?.title} · <span style={{ fontFamily: 'monospace' }}>#{id?.slice(-6)}</span>
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            {polling && <span style={{ fontSize: '0.8125rem', color: 'var(--primary)', display: 'flex', alignItems: 'center', gap: '0.375rem' }}><Brain size={14} /> Grading…</span>}
            {submission.status === 'Pending' && user?.role === 'Teacher' && (
              <button className="btn btn-primary btn-sm" onClick={async () => { try { await axios.post(`/submissions/evaluate/${id}`); toast.success('Evaluation started!'); fetchStatus(); } catch { toast.error('Failed'); } }}>
                Start AI Evaluation
              </button>
            )}
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
              ...
              <div className="card" style={{ padding: '1.75rem', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '3.25rem', fontWeight: 900, margin: 0, letterSpacing: '-0.04em', lineHeight: 1 }} className="gradient-text">
                  {submission.totalMarks ?? 0}
                </p>
                <p style={{ fontSize: '0.8125rem', color: 'var(--muted)', margin: '0.5rem 0 0' }}>
                  {maxPossible > 0 ? `out of ${maxPossible} pts (${percentage}%)` : 'points'}
                </p>
              ...
              <div className="card" style={{ padding: '1.75rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.8125rem', color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: '0.375rem' }}><Clock size={13} /> Updated</span>
                    <span style={{ fontSize: '0.8125rem', color: 'var(--text-2)', fontWeight: 500 }}>{new Date(submission.updatedAt).toLocaleDateString()}</span>
                  </div>
                  {!submission.evaluatedByAI && submission.overrideJustification && (
                    <div>
                      <p style={{ fontSize: '0.75rem', color: 'var(--muted)', margin: '0 0 0.25rem' }}>Override reason</p>
                      <p style={{ fontSize: '0.8125rem', color: 'var(--text-2)', margin: 0, lineHeight: 1.5 }}>{submission.overrideJustification}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="card" style={{ padding: '1.75rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
                {submission.feedback?.map((fb, i) => {
                  ...
                  const wrong   = isMCQ && !fb.isCorrect;
                  return (
                    <div key={i} style={{ padding: '1.25rem', background: 'var(--bg-elevated)', borderRadius: 12, border: '1px solid var(--border)', display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
                      <div style={{ textAlign: 'center', flexShrink: 0, minWidth: 44 }}>
                        ...
                        <p style={{ fontSize: '1.625rem', fontWeight: 900, color: 'var(--text)', margin: 0, lineHeight: 1 }}>{fb.question || i + 1}</p>
                      </div>
                        ...
                        {isMCQ ? (
                          <>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
                              ...
                              <span style={{ fontSize: '0.8125rem', fontWeight: 600, color: correct ? '#10b981' : '#ef4444' }}>
                                {correct ? 'Correct' : 'Incorrect'}
                              </span>
                              <span className={`badge ${isMCQ ? 'badge-indigo' : 'badge-purple'}`} style={{ marginLeft: 'auto' }}>MCQ</span>
                            </div>
                            ...
                            {fb.correctOption  && <p style={{ fontSize: '0.8rem', color: 'var(--muted)', margin: 0 }}>Correct answer: <strong style={{ color: '#10b981' }}>{fb.correctOption}</strong></p>}
                          </>
                        ) : (
                          <>
        ...
        {(submission.status === 'Pending' || submission.status === 'Processing') && (
          <div className="card" style={{ padding: '5rem 2rem', textAlign: 'center' }}>
            <div style={{ width: 60, height: 60, background: 'rgba(91,94,244,0.1)', borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.25rem' }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: '0.9rem', margin: 0 }}>
              {submission.status === 'Processing' ? 'Usually takes 30–60 seconds. Page refreshes automatically.' : 'Your teacher will start AI evaluation soon.'}
            </p>
          </div>
        )}
      </div>
        ...
        <div className="modal-overlay">
          <div className="modal-box" style={{ padding: '2rem' }}>
            ...
            <form onSubmit={handleOverride} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label className="label">New Total Score</label>
                <input type="number" required value={newScore} onChange={e => setNewScore(e.target.value)} className="input" />
              </div>
              <div>
                <label className="label">Reason for change</label>
                <textarea required rows={3} value={justification} onChange={e => setJustification(e.target.value)} className="input" placeholder="Why are you adjusting the marks?" style={{ resize: 'vertical' }} />
              </div>
