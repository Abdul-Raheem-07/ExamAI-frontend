...
import { PlusCircle, Trash2, ArrowLeft, Save, Brain, ListOrdered, AlignLeft, ChevronDown, X } from 'lucide-react';

const TYPES = [
  { value: 'mcq',           label: 'MCQ',            desc: 'Multiple choice' },
  { value: 'short_answer',  label: 'Short Answer',   desc: 'Brief written'   },
  { value: 'long_answer',   label: 'Long Answer',    desc: 'Detailed written'},
];
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', marginBottom: '0.5rem' }}>
    <button
      ...
      style={{
        ...
        border: `2px solid ${correct ? '#10b981' : 'var(--border)'}`,
    ...
    <span style={{ width: 20, color: 'var(--muted)', fontWeight: 700, fontSize: '0.8rem', textAlign: 'center' }}>
...
const QuestionCard = ({ q, index, total, onChange, onRemove }) => {
  const addOption = () => {
    ...
    onChange(index, 'options', [...q.options, '']);
  };
  const removeOption = (oi) => {
    const opts = q.options.filter((_, i) => i !== oi);
    const correct = q.correctAnswers.filter(c => c !== oi).map(c => c > oi ? c - 1 : c);
    onChange(index, 'options', opts);
    onChange(index, 'correctAnswers', correct);
  };
  const toggleCorrect = (oi) => {
    const set = new Set(q.correctAnswers);
    set.has(oi) ? set.delete(oi) : set.add(oi);
    onChange(index, 'correctAnswers', [...set]);
  };
    ...
    <div className="card" style={{ padding: '1.25rem', marginBottom: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ width: 28, height: 28, background: 'linear-gradient(135deg, #5b5ef4, #7c3aed)', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: '0.8rem', flexShrink: 0 }}>
            {index + 1}
          </div>
          {/* Type selector */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
            ...
            <input
              type="number" min="1" value={q.maxMarks}
              onChange={e => onChange(index, 'maxMarks', e.target.value)}
              className="input"
              style={{ width: 62, textAlign: 'center', padding: '0.35rem 0.5rem', fontSize: '0.875rem' }}
            />
          </div>
          {total > 1 && (
      ...
      <textarea
        ...
        rows={2}
        style={{ marginBottom: q.type === 'mcq' ? '1rem' : 0 }}
      />

      {q.type === 'mcq' && (
        <div style={{ marginTop: '0.75rem' }}>
          <p style={{ fontSize: '0.75rem', color: 'var(--muted)', fontWeight: 600, marginBottom: '0.625rem' }}>
            Options — click ○ to mark correct answer(s)
          </p>
          {q.options.map((opt, oi) => (
            <MCQOptionRow
              key={oi} option={opt} index={oi}
              correct={q.correctAnswers.includes(oi)}
              onText={setOptionText} onToggleCorrect={toggleCorrect}
              onRemove={removeOption} canRemove={q.options.length > 2}
            />
          ))}
          {q.options.length < 6 && (
...
const CreateExam = () => {
  ...
  const [questions,   setQuestions]   = useState([
    { questionText: '', type: 'mcq', maxMarks: 5, options: ['', '', '', ''], correctAnswers: [] },
  ]);
  const [saving, setSaving] = useState(false);
  const navigate = useNavigate();

  const addQuestion = () => {
    setQuestions(prev => [...prev, { questionText: '', type: 'mcq', maxMarks: 5, options: ['', '', '', ''], correctAnswers: [] }]);
  };
    ...
    <div className="page-wrapper">
      <div className="page-content" style={{ maxWidth: 780 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
          <button onClick={() => navigate('/teacher/dashboard')} className="btn btn-ghost" style={{ width: 36, height: 36, padding: 0, justifyContent: 'center', flexShrink: 0 }}>
            <ArrowLeft size={16} />
          </button>
          <div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="card" style={{ padding: '1.5rem', marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
              <div>
                ...
                <input className="input" placeholder="e.g., CS101 Midterm — Data Structures" value={title} onChange={e => setTitle(e.target.value)} />
              </div>
              <div>
                <label className="label">Description <span style={{ color: 'var(--subtle)', fontWeight: 400 }}>(optional)</span></label>
                <textarea className="input" rows={2} placeholder="Short description for students…" value={description} onChange={e => setDescription(e.target.value)} />
              </div>
              <div>
                <label className="label">AI Grading Rubric / Answer Key *</label>
                <textarea className="input" rows={3} placeholder="Describe how AI should grade: key concepts, expected points, partial marks rules…" value={rubric} onChange={e => setRubric(e.target.value)} />
              </div>
            </div>
          </div>
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            ...
            {questions.map((q, i) => (
              <QuestionCard key={i} q={q} index={i} total={questions.length} onChange={changeQuestion} onRemove={removeQuestion} />
            ))}
          </div>
          ...
          <button type="submit" disabled={saving} className="btn btn-primary" style={{ width: '100%', padding: '0.9rem', justifyContent: 'center', fontSize: '0.9375rem' }}>
            {saving ? <><span className="animate-spin" style={{ display: 'inline-block', width: 16, height: 16, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: '#fff', borderRadius: '50%' }} />  Saving…</> : <><Save size={16} /> Save Exam</>}
          </button>
        </form>
      </div>
    </div>
